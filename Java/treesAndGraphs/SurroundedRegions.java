...
public class SurroundedRegions {
	...
	public void solve(char[][] board) {
        ...
        for (int i = 0; i < board[0].length; i++) {
            if (board[0][i] == 'O' && !visited[0][i]) {// starting from the elements from FIRST ROW
                helper(board, 0, i, visited);
            }
            if (board[board.length - 1][i] == 'O' && !visited[board.length - 1][i]) { // starting from the elements from LAST ROW
                helper(board, board.length - 1, i, visited);
            }
        }
        for (int i = 0; i < board.length; i++) {
            if (board[i][0] == 'O' && !visited[i][0]) {// starting from the elements from FIRST COL
                helper(board, i, 0, visited);
            }
            if (board[i][board[0].length - 1] == 'O' && !visited[i][board[0].length - 1]) {// starting from the elements from LAST COL
