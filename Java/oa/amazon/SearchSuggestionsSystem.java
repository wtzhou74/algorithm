...
public class SearchSuggestionsSystem {
	...
	public class TrieNode {
        String val;
        Map<Character, TrieNode> children; // 当前child自己的character值以及Node
        boolean isWord;
        
        public TrieNode() {
            this.val = ""; // 这个一般适用来存放叶子节点的值，中间节点不设置值，每个节点中的字符值放在Map.key中
    ...
    public TrieNode buildTrie(String[] products) {
        TrieNode root = new TrieNode(); // 对 Trie结构，root的值初始化为空，从其children开始insert/search
        //TrieNode curr = root;
        for (String product : products) {
        	TrieNode curr = root; // reset curr for next word
            for (int i = 0; i < product.length(); i++) {
                if (!curr.children.containsKey(product.charAt(i))) { // 接下来的char是否更当前字符一样，
                										//  不一样，入map, 形成新的点/分支
...
//    }
    ...
    // 查找具有相同 prefix的所有 products
    public List<String> search(TrieNode root, String prefix) {
        List<String> set = new ArrayList<>(); // 防止加进来的重复
        if (root == null) return set;
        TrieNode curr = root;
        int i = 0;
        for (char c : prefix.toCharArray()) {
            if (curr == null) return set; // 为空了，说明没有word具有这样的prefix
            if (curr.children.containsKey(c)) {
            	//NOTE: i == prefix.length() - 1 MUST BE ADDED
            	// Otherwise: prefix = "baggage", word = "bag", the "bag" will be added to the final result which CANNOT BE matched word, unless prefix = "bag" or "ba" or "b"
...
//                }                    
                curr = curr.children.get(c);
                // 下面如果这样就是错误的，对 “mou”, "mouse", 到 mou 的isWord=true, 此时 curr指向 u 了，
                								/// 递归的时候，看到 u 对应的isWord=true，就添加到list了
//              if (curr.isWord)
//              	return Arrays.asList(curr.val);
            }
            else 
                return set; // 也是一样，没有word有这样的prefix
            i++;
        }
        // DFS to get all matched strings
        // 一定要判断 curr是否为空； DFS 开始遍历每个点，找到 isWord==true的点记录下来
...
//            }
    ...
    public void dfs (TrieNode curr, List<String> words) {
    	if (curr == null) {
            return;
        }
        if (curr.isWord) {
    ...
    public static void main(String[] args) {
    	String[] products = new String[] {"mou","mouse"};
