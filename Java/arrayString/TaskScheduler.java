...
//	                if (count == n) break;
//	            }
//	            if (count < n && total < tasks.length) idle += (n - count);
	      //return tasks.length + idle;
	        ...
	        while (map[25] > 0) {
	            ...
	            Arrays.sort(map);
	            if (count < n && map[25] != 0) res += (n - count);  
	            Arrays.sort(map); // SORT!!!!!! again since the array is not in order anymore after processing
	        }
	        return res;
	        
	   }
	 
	 // Applying PriorityQueue to sort
	 public int priorityQueueSol(char[] tasks, int n) {
		 if (n == 0) return tasks.length;
	        int[] map = new int[26];
	        for (char task : tasks) {
	            map[task - 'A']++;
	        ...
	        for (int occurence : map) {
	            if (occurence > 0) {
	                queue.offer(occurence);
	            }
	        }
	        
	        while (!queue.isEmpty()) {
	            List<Integer> temp = new ArrayList<>();// store new occurrence
	            int i = 0;
	            while (i <= n) {
	                if (!queue.isEmpty()) {// get IDLE when queue is Empty and i <= n
	                    if (queue.peek() > 1) {
	                ...
	                res++;
	                if (temp.size() == 0 && queue.isEmpty()) break;
	                i++;
	            }
...
//		 };
		 ...
		 //new TaskScheduler().leastInterval(t, 1);
	 }
